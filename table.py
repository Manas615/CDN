...
class AdaptiveCDN:
    def __init__(self, num_servers=5, cache_capacity=100, num_content=20):
        self.network = self.create_network(num_servers)
        self.cache = ViralLFUCache(cache_capacity)
        self.router = EngagementAwareRouter(self.network)
    ...
    def run_simulation(self, num_requests=1000):
    ...
    def process_request(self, content_id, engagement_score, path):
        ...
        if is_available:
            ...
            if engagement_score > self.viral_threshold:
                ...
                self.cache.put(content_id, engagement_score)
            else:
                self.cache.put(content_id, engagement_score)

        delivery_time = self.calculate_delivery_time(path)
    ...
    def calculate_delivery_time(self, path):
        total_time = 0
...
class ViralLFUCache:
    ...
    def put(self, key, engagement_score):
        if len(self.cache) >= self.capacity:
            ...
            if engagement_score > self.viral_threshold:
                ...
                return
        self.cache[key] = engagement_score
        self.frequency[key] += 1
        if engagement_score > self.viral_threshold:
...
class DataGenerator:
    def __init__(self, num_content=20):
        self.num_content = num_content
...
if __name__ == "__main__":
    cdn_simulation = AdaptiveCDN(num_servers=5, cache_capacity=50, num_content=20)
    cdn_simulation.run_simulation(num_requests=1000)
